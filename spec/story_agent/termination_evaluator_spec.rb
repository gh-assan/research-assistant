...
RSpec.describe ResearchAssistant::StoryAgent::TerminationEvaluator do
  let(:termination_evaluator) { described_class.new }
  let(:max_iterations) { 5 }

  before do
    allow(ResearchAssistant.config).to receive(:max_story_agent_iterations).and_return(max_iterations)
  end
  ...
  describe '#max_iterations' do
    it 'returns the max_story_agent_iterations from the configuration' do
