...
RSpec.describe ResearchAssistant::CoreEngine::GapDetector do
  ...
  let(:response) { "Detected gaps" }

  before do
    allow(ResearchAssistant::OllamaInterface::ApiClient).to receive(:new).with('research-assistant-gaps-extractor-model').and_return(api_client)
    allow(api_client).to receive(:query).and_return(response)
  end

  describe '#extract' do
    it 'detects gaps from the given topic and article' do
      result = detector.extract(topic, article)
    ...
    context 'when the API request fails' do
      it 'raises an API error' do
        ...
        expect { detector.extract(topic, article) }.to raise_error RuntimeError, 'API Error: Something went wrong'
      end
    end
    ...
    context 'when there is a connection error' do
      it 'raises a connection error' do
        ...
        expect { detector.extract(topic, article) }.to raise_error(RuntimeError, 'Connection Error: Connection failed')
      end
    end
  end
end